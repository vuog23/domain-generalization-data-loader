...
def get_weight_path(hparams):
    arch = hparams["arch"]
    pretrain = hparams["pretrain"]
    if hparams.get("pretrained_weights_dir"):
        return os.path.join(
            os.path.expanduser(hparams["pretrained_weights_dir"]),
            os.path.basename(weights_path[arch][pretrain]),
        )
    return weights_path[arch][pretrain]
    ...
    def __init__(self, input_shape, hparams):
        ...
        if hparams["arch"] == 'resnet18':
            ...
            if hparams["pretrain"] == "Supervised":
                ...
                self.network = torchvision.models.resnet50(pretrained=False)
                weight_path = get_weight_path(hparams)
                ...
                assert os.path.isfile(weight_path)
                checkpoint = torch.load(weight_path, map_location="cpu")
                self.network, _ = simclr_resnet.get_resnet(*simclr_resnet.name_to_params(os.path.basename(weight_path)))
